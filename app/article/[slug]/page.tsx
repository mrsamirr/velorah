...
export default async function ArticleSlugPage({
  ...
  const [comments, liked, bookmarked] = await Promise.all([
    ...
    user ? isArticleLiked(article.id).catch(() => false) : Promise.resolve(false),
    user ? isBookmarked(article.id).catch(() => false) : Promise.resolve(false),
  ]);
          ...
          <div className="space-y-8 mb-16">
            ...
            <div className="flex items-center justify-between py-8 border-y border-white/5">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <Link
                    href={`/author/${article.author_username ?? article.author_id}`}
            ...
            <div className="flex items-center gap-3">
              {user ? (
                <>
                  <LikeButton articleId={article.id} liked={liked} />
                  <BookmarkButton articleId={article.id} bookmarked={bookmarked} />
                </>
              ) : (
                <>
                  <Link
          ...
          <section className="border-t border-white/5 pt-16 mb-20">
            ...
            <div className="mt-12 pt-8 border-t border-white/5">
              {user ? (
                ...
                <p className="text-on-surface-variant text-sm">
                  <Link href={`/signin?redirectTo=/article/${article.slug}`} className="text-on-surface underline underline-offset-4">
                    Sign in
                  </Link>{" "}
