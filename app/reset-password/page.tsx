...
export default function ResetPasswordPage() {
  ...
  // Supabase delivers the recovery token via URL hash fragment.
  // onAuthStateChange fires a PASSWORD_RECOVERY event once the SDK
  // exchanges it for a session — only then is updateUser() safe to call.
  useEffect(() => {
    ...
    // If no event fires after 5 s the link is invalid/expired
    const timer = setTimeout(() => {
      setInvalid((prev) => { if (!prev && !ready) return true; return prev; });
    }, 5000);
    return () => {
      ...
      <section className="flex-1 flex items-center justify-center px-6 py-24 md:p-12 lg:p-24">
        <div className="w-full max-w-md glass-panel p-10 border border-outline-variant/20">
          <header className="mb-12">
            <h2 className="font-headline text-5xl text-white mb-2">New Password</h2>
            <p className="text-outline font-body text-sm">Choose a strong password for your account.</p>
          </header>

          {invalid && !ready ? (
            <div>
              <p className="mb-6 text-sm text-error bg-error-container/20 border border-error/20 px-4 py-3">
            ...
            <>
              ...
              <form className="space-y-8" action={formAction}>
                ...
                <div className="space-y-2">
                  <Label htmlFor="confirmPassword" className={state?.errors?.confirmPassword ? "text-error" : ""}>
                  ...
                  {state?.errors?.confirmPassword && (
                    <p className="text-xs text-error mt-1">{state.errors.confirmPassword[0]}</p>
                  )}
                </div>
                ...
                <div className="pt-4">
                  <Button
                    type="submit"
                    disabled={pending || (!ready && !invalid)}
                  ...
                  >
                    {pending ? "Saving…" : !ready ? "Verifying link…" : "Set New Password"}
                  </Button>
                </div>
              </form>
            </>
